...
def main() -> int:
    ...
    sql = "\n".join(
        [
            ...
            ")",
            'DELETE FROM "__ARP_SCHEMA__".reports WHERE activity_id IN (SELECT activity_id FROM affected);',
            'DELETE FROM "__ARP_SCHEMA__".sources WHERE activity_id IN (SELECT activity_id FROM affected);',
            "",
            ...
            "",
            "-- Ensure documents rows exist for all sources",
            'INSERT INTO "__ARP_SCHEMA__".documents (source_id)',
            'SELECT s.source_id FROM "__ARP_SCHEMA__".sources s',
            'WHERE s.activity_id IN (SELECT activity_id FROM affected)',
