...
CREATE TABLE IF NOT EXISTS ops.documents (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  -- Optional legacy grouping (kept for backward compatibility; UI may display it as a prefix).
...
CREATE INDEX IF NOT EXISTS documents_app_group_idx ON ops.documents(app_key, group_key, updated_at DESC);
